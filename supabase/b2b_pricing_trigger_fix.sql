...
begin
    if not exists (select 1 from information_schema.columns where table_name = 'price_lists' and column_name = 'type') then
        ...
        -- Opsiyonel: Mevcut kayıtlara varsayılan değer ata (örn: 'b2c' veya 'standard')
        -- update price_lists set type = 'standard' where type is null;
    end if;
end $$;


-- 2. App Settings Tablosu (Global Ayarlar)
...
-- 3. B2B Fiyat Listesi ID'sini Getiren Yardımcı Fonksiyon (GÜNCELLENDİ)
...
declare
    ...
    settings_json jsonb;
begin
    -- Sadece base_price değiştiyse işlem yap
...
-- 5. Trigger Fonksiyonu: GLOBAL Yüzde Değişince (Tümünü Güncelle)
